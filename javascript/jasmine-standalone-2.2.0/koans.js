...
describe("the JavaScript language", function(){
  ...
  describe("has different types and operators that", function(){
    ...
    it("considers nulls to be falsy", function(){
        ...
        expect(null == false).toBeFalsy();// Truthy or Falsy ===== WHY!!??
    });
    ...
    it("has arrays and they can contain anything inside", function(){
        ...
        expect(arr[7]).toEqual(undefined);
...
// take note
  ...
  describe("considers functions as first class citizens", function(){
    ...
    it("may return arrays that contains functions and so on", function(){
        function example(){
           ...
           return [function(arg){ return [1, 10]; }, 3, 5];
        }
    ...
    it("matters, the declaration order when they are anonymous", function(){
        ...
        //expect(exampleA()).toEqual(undefined); --> this returns type error as exampleB is not (yet) a function.
    ...
    it("anonymous functions are anonymous", function(){
         ...
         expect(typeof(z)).toEqual('undefined');  // --> you'd expect z to be a function, but if it's inside a variable, z doesn't mean shit! Refer to the following line:
    ...
    it("may return arrays that contains closures and so on", function(){
        function example(){
           // write the missing code here
           var a = 10
           return [function(arg){ return [0, 9+arg]}, 4, 5, 'whatever']
        }
    ...
    it("passes primitive types as values (a copy) to functions", function(){
        ...
        example(x)
        expect(x).toEqual(1);
        expect(example(x)).toEqual(undefined) // testing...
    ...
    it("can invoke functions indirectly using the special 'call'", function(){
        ...
        expect(f.call(null,1,1)).toEqual(2);
    });
    // MIND FUCK
    ...
    it("can invoke functions indirectly using the special 'apply'", function(){
        ...
        expect(f.apply(f, [3,5])).toEqual(8);
        //expect(f.apply(f, 3, 2)).toEqual(5); // --> type error
    });
  ...
  describe("has multiple ways to define and create objects", function(){
    ...
    it("may define complex objects", function(){
        var user;
        // write the contents of the obj to make satisfy the expectations:
        user = {address: {street: 'sesame'}, friends: [{name: 'triki'}, 'whatever']};
    ...
    it("may create objects also with the module pattern", function(){
        function createObject(initialScore){
            ...
            return {
                incrementScoreIn: function(anotherScore){
...
//things to learn here: 'constructor' (constructor is function capitalized), 'this' --> similarities to Ruby instance method and class methods
    ...
    it("can have have methods in the prototype", function(){
        function Obj(){
          var name = 'bob';
          // this.name = name; // --> this would be a way to access the value
        }
